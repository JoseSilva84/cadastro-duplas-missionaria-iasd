...
export function exportarLeadPdf(lead, igrejas = []) {
  ...
  const dados = [
    ...
    ['Material recebido', texto(lead.materialRecebido)],
    ['Materiais recebidos', texto(lead.materiaisQuantidade, '0')],
    ['Similaridade VIP', Number.isFinite(Number(lead.similaridadeVip)) ? `${Math.round(Number(lead.similaridadeVip) <= 1 ? Number(lead.similaridadeVip) * 100 : Number(lead.similaridadeVip))}%` : 'Não informada'],
    ['Faixa', texto(lead.faixa)],
    ['Último contato', dataCurta(lead.dataUltimoContato)],
    ['Dias sem contato', texto(lead.diasSemContato)],
    ['Precisão da localização', texto(lead.geoPrecisao)],
